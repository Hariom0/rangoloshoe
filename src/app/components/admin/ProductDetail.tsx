	...
	return (
		<>
			<Sheet open={open} onOpenChange={onOpenChange}>
				<SheetContent side="bottom" className="h-[92vh] sm:max-h-[85vh] sm:max-w-[420px] sm:mx-auto w-full overflow-y-auto rounded-t-3xl sm:rounded-3xl p-0 border-t sm:border shadow-2xl sm:mb-4">
					...
					<div className="relative aspect-square w-full bg-black/5 flex items-center justify-center overflow-hidden">
						{activeMedia ? (
							isVideo ? (
								<video src={activeMedia.url} controls className="h-full w-full object-contain bg-black" autoPlay muted />
							) : (
								<img src={activeMedia.url} alt={activeMedia.altText || product.name} className="h-full w-full object-cover" />
							)
						) : (
								...
								return (
									...
									>
										{isThumbVideo ? (
											...
											<img src={m.url} alt={m.altText || `Thumbnail ${i + 1}`} className="h-full w-full object-cover" />
										)}
									</button>
								);
							})}
					...
					<div className="space-y-6 p-5">
						...
						<div className="flex items-end gap-3 rounded-xl bg-muted/30 p-4 border">
							<span className="text-3xl font-bold text-foreground leading-none">₹{hasDiscount ? product.discountPrice : product.price}</span>
							{hasDiscount && (
								<>
									<span className="text-lg text-muted-foreground line-through leading-none mb-1">₹{product.price}</span>
						...
						<div className="space-y-3">
							...
							<div className="grid grid-cols-2 gap-2">
								{(product.variants || []).map((v: any, i: number) => (
									<div key={v._id || i} className="flex flex-col justify-center rounded-xl border bg-card p-3 shadow-sm">
										<div className="flex items-center justify-between mb-1">
											<div className="text-sm font-bold">Size {v.size}</div>
											<Badge variant={v.stock === 0 ? "destructive" : "secondary"} className="text-[10px]">
												{v.stock} qty
											</Badge>
										</div>
						...
						<div className="flex flex-col gap-1 text-[11px] text-muted-foreground border-t pt-4">
							<div className="flex items-center gap-1.5">
								...
								<span>Created: {product.createdAt ? new Date(product.createdAt as string | number | Date).toLocaleDateString() : "N/A"}</span>
							</div>
							<div className="flex items-center gap-1.5">
								<Calendar className="h-3 w-3" />
								<span>Last updated: {product.updatedAt ? new Date(product.updatedAt as string | number | Date).toLocaleDateString() : "N/A"}</span>
							</div>
						</div>
						...
						<Button variant="ghost" onClick={() => setConfirmOpen(true)} className="h-12 w-full text-destructive hover:bg-destructive/10 hover:text-destructive rounded-xl mt-2">
			...
			<AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
				<AlertDialogContent className="sm:max-w-[400px] rounded-2xl">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>This permanently removes the product, its variants, and all attached media. This action cannot be undone.</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>

...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
	try {
		...
		const product = await Footware.find({ slug: id }); // Note: If finding by slug and expecting one, findOne() is better than find()
		if (!product) {
		...
		// 4. Save to Database
		// Note: Make sure to use findOneAndUpdate if you are querying by a unique 'slug' rather than '_id'
